...
\begin{center}
{\Large\bfseries A Short Path Joining Two Zeros Inside a Polynomial Lemniscate}
\end{center}
...
\noindent The proof of Theorem~1 has three parts.
\begin{enumerate}
...
\]
Thus $v_{\varepsilon,t}$ is admissible for the condenser $(K_t,U_\varepsilon)$.

We now justify directly that $v_{\varepsilon,t}$ minimizes the Dirichlet energy. Let $w$ be any admissible competitor for $(K_t,U_\varepsilon)$. Replacing $w$ by $\min\{1,\max\{w,0\}\}$ if necessary, we may assume $0\le w\le 1$. On $\Omega_{\varepsilon,t}$ set
\[
\eta:=w-v_{\varepsilon,t}.
\]
Since both $w$ and $v_{\varepsilon,t}$ equal $0$ on $\partial U_\varepsilon$ and equal $1$ on $\partial K_t=\{u=t\}$, the function $\eta$ has zero trace on $\partial\Omega_{\varepsilon,t}$. Because $u$ is harmonic on $\Omega_{\varepsilon,t}$, so is $v_{\varepsilon,t}$. Hence
\[
...
\int_{\Omega_{\varepsilon,t}} |\nabla\eta|^2\,dA
+
2\int_{\Omega_{\varepsilon,t}} \nabla v_{\varepsilon,t}\cdot \nabla\eta\,dA.
\]
The last term vanishes by integration by parts:
\[
\int_{\Omega_{\varepsilon,t}} \nabla v_{\varepsilon,t}\cdot \nabla\eta\,dA
=
-\int_{\Omega_{\varepsilon,t}} \eta\,\Delta v_{\varepsilon,t}\,dA
+
\int_{\partial\Omega_{\varepsilon,t}} \eta\,\partial_\nu v_{\varepsilon,t}\,ds
=0,
\]
because $\Delta v_{\varepsilon,t}=0$ on $\Omega_{\varepsilon,t}$ and $\eta|_{\partial\Omega_{\varepsilon,t}}=0$. Therefore
\[
\int_{\Omega_{\varepsilon,t}} |\nabla w|^2\,dA
\ge
\int_{\Omega_{\varepsilon,t}} |\nabla v_{\varepsilon,t}|^2\,dA.
\]
Since the total Dirichlet energy of $w$ on $U_\varepsilon$ is at least its energy on $\Omega_{\varepsilon,t}$, it follows that $v_{\varepsilon,t}$ is the capacitary potential of $(K_t,U_\varepsilon)$. With the normalization
\[
...
\]
By Lemma~3, \(\partial U\subset \{|f|=1\}\) has planar measure zero, so
\[
...
\[
 M_s:=U\cap\{e^{-t}\le |f|\le e^{-s/2}\}.
\]
As in the proof of Lemma~6, $M_s$ is compact and contained in $\{s/2\le u\le t\}$, so the vector field
\[
 X=\frac{\nabla u}{|\nabla u|^2}
\]
...
\end{cases}
\]
As in Lemma~6, the trajectory segments remain in the compact slab $M_s$, so standard
...
\begin{lemma}[Connected regular regions are strips]
...
\medskip
\begin{lemma}[Regular lemniscate components are Jordan domains]
...
\medskip
\begin{lemma}[Regular-slab deformation and connectivity]
...
\item[(c)] \emph{if moreover $a$ and $b$ are regular values, then every connected component of $V\cap\{a<u<b\}$ is
 a regular strip in the sense of Lemma~8.}
\end{enumerate}
\end{lemma}
...
\]
so the side-boundary hypothesis of Lemma~8 is vacuous. Applying Lemma~8 to $S$ gives the desired
...
\medskip
\begin{lemma}[Geometric realization of a finite tree]
...
\emph{Let $c\ge 0$, and let $V$ be a connected component of $\{u>c\}$ containing exactly $m$
...
Let $z\in V$ with $u(z)<2\alpha$, and write $s_z:=u(z)\in(c,2\alpha)$. Set
\[
M_z:=V\cap\{s_z\le u\le 2\alpha\}.
\]
Because $V$ is a connected component of the open set $\{u>c\}$, it is relatively closed there; since $s_z>c$, the set $M_z$ is closed in $\mathbb{C}$. It is also bounded because $\Lambda_f$ is bounded, hence compact. As $u$ has no critical points on $M_z$, the vector field
\[
X=\frac{\nabla u}{|\nabla u|^2}
\]
is smooth on a neighborhood of $M_z$. Along the maximal trajectory $\phi_\tau(z)$ through $z$ one has
\[
u(\phi_\tau(z))=s_z+\tau,
\]
so the trajectory stays in $M_z$ for $0\le \tau\le 2\alpha-s_z$ and therefore exists on that whole interval. Thus every point of $V$ flows inside $V$ to a point of $V_{\ge 2\alpha}$. Defining the homotopy by
flowing each point upward until it reaches level $2\alpha$ (and then keeping it fixed), one obtains,
by the same continuity argument used in Lemma~10(a) on each compact subslab
$V\cap\{s\le u\le 2\alpha\}$ with $c<s<2\alpha$, a strong deformation retraction of $V$ onto
$V_{\ge 2\alpha}$. Since $V$ is connected, $V_{\ge 2\alpha}$ is connected.

Now let $T>2\alpha$ be a sufficiently large regular value such that $\{z\in V:u(z)>T\}$ has exactly $m$ connected components, one near each zero of $f$ in $V$. Since there are no critical values of $u$ in $[2\alpha,T]$, Lemma~10(a) gives a strong deformation retraction of $V_{\ge 2\alpha}$ onto
\[
...
\]
so small that each $I_r$ contains no critical value of $u$ other than $\mu_r$. Choose a regular value
\[
 \tau_0\in(2\alpha,\mu_1-\delta_1),
\]
and choose a regular value
\[
 T>\mu_s+\delta_s
\]
...
\[
 Z_1,\dots,Z_m
\]
of $\{z\in V:u(z)>T\}$ have pairwise disjoint closures. 
...
\]
so $V_{\ge t_0}$ is connected. Since there are no critical points in $\{z\in V:t_0\le u\le \tau_0\}$, Lemma~10(a) shows
that
\[
V_{\ge \tau_0}:=\{z\in V:u\ge \tau_0\}
\]
is connected. Next choose a regular value $\tau_1\in(\tau_0,\mu_1-\delta_1)$. Applying Lemma~10(a) on $[\tau_0,\tau_1]$,
we find that $V_{\ge \tau_1}$ is connected. Lemma~10(b) applied with $a=\tau_0$ and $b=\tau_1$ now gives that
...
We shall prove below that, after shrinking the neighborhoods $N_r$ further if
necessary, each of the cross-sections $C_r^{-}, C_{r,1}^{+}, C_{r,2}^{+}$
...
component of $S$ is a flow line of $X$, Lemma~8 applies: $S$ is a regular strip, with one connected lower
...
Lemma~9 implies that $\partial C_t(S)$ is a smooth Jordan curve contained in
$\{u=t\}\cap V$.
...
\]
where $\Phi_{s_0}$ is the strip diffeomorphism from Lemma~8. We claim that $\widetilde\gamma_x$ extends continuously to
...
\indent Summing over all strips and using $k_S\ge 1$, we obtain
\[
\sum_S \operatorname{len}(\gamma_S)\le \frac{1}{2\pi}\sum_S\int_{a_S}^{b_S} P_S(t)\,dt.
\]
Since the strips are disjoint and
\[
\sum_S P_S(t)=\mathcal H^1(\{u=t\}\cap\Omega)\le P_V(t)
\]
for every regular $t\in(\tau_0,T)$, Fubini's theorem yields
\begin{equation}
\sum_S \operatorname{len}(\gamma_S)\le \frac{1}{2\pi}\int_{\tau_0}^{T} P_V(t)\,dt\le \frac{1}{2\pi}\int_{2\alpha}^{\infty} P_V(t)\,dt.
\tag{11}
\end{equation}
...
\]
Lemma~10(a), applied on $[\tau_0,\mu_1-\delta_1]$, shows that
\[
...
\]
Applying Lemma~10(a) on $[\mu_1-\delta_1,\tau_1]$ shows that
$V^{\ge \tau_1}$ is connected. Then Lemma~10(b), applied with
...
respectively. Hence Lemma~9 applies to both of them, so each is a Jordan domain. Since
\[
...
\]
Therefore $A_0$ is a connected annulus. Since there are no critical points on $A_0$, Lemma~10(c) implies that every connected component of $A_0$ is a regular strip. Because $A_0$ is connected, it is a single regular strip. Hence there is exactly one strip whose lower cross-section lies in $\{u=\tau_0\}$; denote it by $S_0$.

\smallskip
\noindent\emph{Critical neighborhoods.}
\medskip
\noindent\emph{Claim.}
After shrinking the pairwise disjoint neighborhoods $N_r$ further if necessary, each of the cross-sections
...
Fix $r$. Choose Morse coordinates $(x,y)$ on a neighborhood $U_r$ of $p_r$ such that
\[u=\mu_r+x^2-y^2,
\]
and shrink $N_r\Subset U_r$ so that the three cross-sections
\[
C_r^- \subset \{u=\mu_r-\delta_r\},\qquad
C_{r,1}^+,\,C_{r,2}^+\subset \{u=\mu_r+\delta_r\}
\]
appear exactly as in the standard saddle picture, with the remainder of $\partial N_r$
consisting of flow segments of $X$.

In these coordinates, each of the three cross-sections carries a one-sided collar in
$U_r\setminus N_r$ contained in $\Omega$: the collar adjacent to $C_r^-$ lies on the
lower side of the saddle, while the collars adjacent to $C_{r,1}^+$ and $C_{r,2}^+$ lie on
the two upper sides. Each such collar is connected. Therefore each cross-section is
contained in the boundary of exactly one connected component of $\Omega$; denote these
components by
\[
S_r^-,\qquad S_{r,1}^+,\qquad S_{r,2}^+.
\]

The same local coordinate picture shows that $C_r^-$ is the upper cross-section of
$S_r^-$, while $C_{r,1}^+$ and $C_{r,2}^+$ are the lower cross-sections of
$S_{r,1}^+$ and $S_{r,2}^+$, respectively. This proves the claim.
\hfill$\triangle$
\smallskip
...
Y_j:=W_j\cap\{u\ge T\}.
\]
By the component-inheritance paragraph above, $W_j$ is also a connected component of the global regular superlevel set $\{u>c_j\}$. Hence Lemma~9 implies that $W_j$ is a Jordan domain.

We claim that $Y_j$ is connected. Define
\[
R_j(z):=
\begin{cases}
z, & u(z)\ge T,\\
\varphi_{T-u(z)}(z), & c_j<u(z)<T,
\end{cases}
\]
and
\[
...
\begin{cases}
z, & u(z)\ge T,\\
\varphi_{\theta(T-u(z))}(z), & c_j<u(z)<T.
\end{cases}
\]
If $z_0\in W_j\setminus Y_j$, then the trajectory segment
\[
\{\varphi_\tau(z_0):0\le \tau\le T-u(z_0)\}
\]
stays in the compact set
\[
W_j\cap\{u(z_0)\le u\le T\},
\]
on which $X$ is smooth because there are no critical points on $W_j\cap\{c_j<u\le T\}$. Standard continuous dependence for smooth ODEs therefore shows that $H_j$ is continuous on $[0,1]\times W_j$. Thus $H_j$ is a strong deformation retraction of $W_j$ onto $Y_j$, and $Y_j$ is connected.
...
\]
Therefore $A_j$ is a connected annulus. Since there are no critical points on $A_j$, Lemma~10(c) implies that every connected component of $A_j$ is a regular strip. Because $A_j$ is connected, it is a single regular strip. Hence there is exactly one strip of $\Omega$ whose upper cross-section is $\partial K_j$; denote it by $S_j^{\mathrm{top}}$.
...
\smallskip
\noindent\emph{Proof.}
Let $S$ be a connected component of $\Omega$. By Lemma~8, $S$ is a regular strip,
...
$v_0$. Thus every vertex is connected to $v_0$, and the graph is connected and acyclic.
Therefore $T$ is a finite rooted tree.
...
C_r^-,\ C_{r,1}^+,\ C_{r,2}^+
\]
are disjoint subsets of \(\partial N_r\). Choose a point \(c_r\in \operatorname{int}(N_r)\). Because \(N_r\) is a closed topological disk, there exists an embedded \(Y\)-shaped tree
\[
T_r\subset N_r
\]
with branch point \(c_r\) and three terminal vertices \(x_{r,-},x_{r,1},x_{r,2}\). Since the neighborhoods \(N_r\) may be chosen arbitrarily small at the outset and are pairwise disjoint, we may assume in addition that
\[
\sum_{r=1}^s \operatorname{len}(T_r)<\frac{\varepsilon}{2}. \tag{12}
\]

Let \(x_0\) be the lower endpoint of \(\gamma_{S_0}\) on the level \(\{u=\tau_0\}\). For each \(j\), let \(b_j\) be the upper endpoint of \(\gamma_{S_j^{\mathrm{top}}}\) on \(\partial K_j\). Write
\[
f(z)=(z-a_j)^{m_j}h_j(z)
\]
in a neighborhood of \(a_j\), where \(m_j\ge 1\) and \(h_j\) is holomorphic with \(h_j(a_j)\neq 0\). After shrinking to a sufficiently small closed disk \(B(a_j,r_j)\), choose a holomorphic branch \(q_j\) of \(h_j^{1/m_j}\) there and define
\[
\psi_j(z):=(z-a_j)q_j(z).
\]
Then \(\psi_j\) is biholomorphic on a neighborhood of \(B(a_j,r_j)\), one has \(\psi_j(a_j)=0\), and
\[
f(z)=\psi_j(z)^{m_j}
\qquad (z\in B(a_j,r_j)).
\]
By the choice of $T$, each $K_j$ is contained in $B(a_j,r_j)$ and
\[
Z_j=\psi_j^{-1}\bigl(D(0,e^{-T/m_j})\bigr),
\qquad
K_j=\psi_j^{-1}\bigl(\overline{D(0,e^{-T/m_j})}\bigr).
\]
...
\]
joining $a_j$ to $b_j$. Since $\psi_j^{-1}$ is $C^1$ near $0$, there exists $C_j>0$ such that
\[
\operatorname{len}(\sigma_j)\le C_j e^{-T/m_j}.
\]
Hence, by the choice of \(T\),
\[
...
\]
We verify the hypotheses of Lemma~11 for the family of arcs $\{A_S\}$. First, if $S\neq S'$, then
\[
\gamma_S\cap\gamma_{S'}=\varnothing,
\]
because distinct strips are disjoint subsets of $\Omega$. Second, for each $r$, the tree $T_r\subset N_r$ meets the rest of the construction only at its terminal points $x_{r,-},x_{r,1},x_{r,2}$: indeed the neighborhoods $N_r$ are pairwise disjoint, each $N_r$ is disjoint from every $Z_j$, and a strip trajectory can meet $\partial N_r$ only at the cross-section belonging to that strip. Third, for each $j$, the arc $\sigma_j\subset K_j\subset Z_j$ meets the rest of the construction only at its endpoint $b_j\in\partial K_j$. Indeed, the sets $Z_1,\dots,Z_m$ are pairwise disjoint and disjoint from all $N_r$, so the arcs $\sigma_j$ are pairwise disjoint and are disjoint from every $T_r$.

It follows that the interior of each $A_S$ contains no geometric vertex, the interiors of distinct arcs $A_S$ and $A_{S'}$ are disjoint, and two distinct arcs can meet only at a common geometric endpoint corresponding to a common endpoint of the associated edges of $T$. Hence Lemma~11 applies:
\[
 H:=\bigcup_S A_S
\]
...
\]
Then $K$ and $L$ are compact and connected by Lemma~7.
...
\]
Since $L$ is connected by Lemma~7 and the $\overline{W_i}$ are pairwise disjoint closed subsets of $L$, it follows
that $r=1$. Let $W:=W_1$. Because $W\subset U\subset\{u>0\}$ and $c>0$, the set $W$ is also a connected
component of the global superlevel set $\{u>c\}$. Hence Lemma~9 implies that $W$ is a Jordan domain and
\[
L=\overline W.
\]
...
\]
Since $K$ is connected by Lemma~7, we must have $s=1$. Let $Y:=Y_1$. Because $d>0$ and $Y\subset U\subset\{u>0\}$,
this set $Y$ is a connected component of the global superlevel set $\{u>d\}$. Hence Lemma~9 implies that $Y$
...
$\zeta\in Z_K$, so small that
\[
|f(z)|\le M_*+\frac{\eta_*}{4}=e^{-2\alpha}-\frac{3\eta_*}{4}
\qquad
(z\in E:=\bigcup_{\zeta\in Z_K}E_\zeta),
\]
...
\]
Equivalently, every critical value of $u_g$ arising from a critical point in $K$ is $>2\alpha$.
In particular, no critical point of $u_g$ in $K$ has critical value in $[\alpha,2\alpha]$. In this case set
\[
\beta_*:=\frac{\eta_*}{2}.
\]

\noindent\textit{Step 5: make the moduli distinct.} Let $a_1,\dots,a_N$ be the pairwise distinct complex critical
...
the disks $D_j$, and by Step~4 every $\beta$ with $|\beta|<\beta_*$ keeps all critical values in $K$ above $2\alpha$. Hence,
with
\[
\beta_0:=\min\{\beta_1,\beta_*\}>0,
\]
we may choose $\beta$ with $0<|\beta|<\beta_0$ outside that finite union of lines. Then the critical values of $u_g$
arising from critical points in $K$ and larger than $2\alpha$ are pairwise distinct.
...
\]
By assumption, $g$ has no zeros on $K\setminus \bigcup_j D_j$, so $-\log|g|$ is harmonic
on an open neighborhood of $K\setminus \bigcup_j D_j$, hence on a neighborhood of $\overline W$.
...
We next identify the relevant collar region for $u_g$. By Lemma~\ref{lem:component-stability},
$V_g\subset K$ and $V_g$ is the unique connected component of $\{u_g>\alpha\}\cap K$.
Since $u_g\to u$ uniformly on $C$, for all sufficiently small perturbations we have
\[
\|u_g-u\|_{L^\infty(C)}<\alpha/2.
\]
Therefore, if $z\in K$ satisfies $\alpha<u_g(z)<2\alpha$, then $z\notin \bigcup_j D_j$, so
$z\in C$, and hence
\[
\alpha/2<u(z)<5\alpha/2.
\]
Therefore $z\in B$. Also, any such $z$ belongs to $\{u_g>\alpha\}\cap K$, hence to the unique
component $V_g$. Thus
\[
...
\]
which has planar measure zero by Lemma~3. Thus $1_{E_g}\to 1_E$ almost everywhere
on $B$.
...
\emph{Let $T$ be a finite tree of total length $L$, and let $x_1,\dots,x_M$ be marked
points of $T$. Then some pair $x_i,x_j$ is joined by a subpath of length at most $2L/M$.}
\end{lemma}

\begin{proof} Subdivide edges at the marked points; this does not change the total length and turns the
marked points into marked vertices. Take the minimal subtree spanning the marked vertices. Every
leaf of this subtree is marked. Doubling every edge produces an Eulerian graph of total length $2L$.
Traversing an Euler tour and recording the first appearances of the marked vertices gives a cyclic
ordering of the $M$ marked vertices. The $M$ gaps between consecutive marked vertices along the
tour have total length $2L$, so one of them has length at most $2L/M$. The corresponding unique
path in the original tree has at most that length. \end{proof}
...
\]
where $\varepsilon_0>0$ is small enough for Lemmas~14 and~15 to apply, and set
\[
g:=g_{\lambda,\beta}.
\]
...
By Lemma~16, some pair of perturbed roots, say \(b_r\in D_r\) and \(b_s\in D_s\), is joined in \(G\) by
a subpath of length at most
\[
\frac{2}{m}\len(G)
<
...
\tag{18}
\]
The indices \(r\neq s\), because the \(m\) perturbed roots are distinct and each disk \(D_j\) contains
exactly one of them.
...
\frac{q}{4m}
\]
by \((17)\). Therefore the resulting curve in \(U\subset \Lambda_f\) joining two zeros of \(f\) has total
