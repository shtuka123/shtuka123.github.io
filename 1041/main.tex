...
Fix $r$. Choose Morse coordinates $(x,y)$ on a neighborhood $U_r$ of
$p_r$ such that
\[
u = \mu_r + x^2 - y^2,
\]
...
By shrinking $N_r$ further if necessary, we may also arrange that there exist connected
...
\quad
\mathcal C_{r,2}^+
\subset (U_r \setminus N_r)\cap \Omega
\]
such that
\[
...
\]
and every point of $(U_r\setminus N_r)\cap\Omega$ sufficiently close to $C_r^-$
(respectively to $C_{r,i}^+$) lies in $\mathcal C_r^-$ (respectively in $\mathcal C_{r,i}^+$).
...
\noindent\textit{Step 5: make the moduli distinct.} Before comparing moduli, we record that no critical point of $g$ in $K$ is a zero of $g$.
Indeed, if $c\in K$ satisfies
\[
g'(c)=0,
\]
then
