...
\]
Thus $v_{\varepsilon,t}$ belongs to $H_0^1(U_\varepsilon)$, has trace $\ge 1$ on $K_t$, and is therefore admissible in the standard $H^1$-formulation of condenser capacity. By the usual density equivalence between the $C_c^\infty$ and $H_0^1$ admissible classes, this is enough for the capacity computation below.

We now justify directly that $v_{\varepsilon,t}$ minimizes the Dirichlet energy among admissible $H_0^1$-competitors. Let $w\in H_0^1(U_\varepsilon)$ satisfy $w\ge 1$ on $K_t$. Composing with the $1$-Lipschitz clipping map $\phi(s):=\min\{1,\max\{s,0\}\}$ does not increase the Dirichlet energy, so replacing $w$ by $\phi\circ w$ if necessary, we may assume $0\le w\le 1$ a.e. On $\Omega_{\varepsilon,t}$ set
\[
...
Assume from now on that $Z_K\neq\varnothing$. Before comparing moduli, we record
...
By Lemma~\ref{lem:tree-lemma}, some pair of perturbed roots, say \(b_r\in D_r\) and \(b_s\in D_s\), is joined in \(G\) by
