...
$$ V_{\ell} := \varinjlim_{D} H^2(\Sht_{2, D}^{\mu} \times \bar{\eta}_2, \bar{\BQ}_{\ell} ) $$ where $\eta_2 = \Spec F_2$ is the generic point of the surface $X^2$, and $\bar{\eta}_2 = \Spec \bar{F_{2}}$. Then $\GL_2(\BA_f)$ acts on $V_{\ell}$. If $\pi$ is a cuspidal automorphic representation of $\GL_2(\BA_f)$ with $\bar{\BQ}_{\ell}$ coefficients, then the representation of $G_{F_2}$ on $\Hom_{ \GL_2(\BA_f) }(\pi, V_{\ell})$ factors through a representation of $W_F \times W_F$ of the form $\varphi \otimes \hat{\varphi}$, where $\varphi$ is irreducible and $2$-dimensional. Then $ \pi \mapsto \varphi$ *is* the Langlands correspondence for $\GL_2$ over $F$.
