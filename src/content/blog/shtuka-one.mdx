...
<MathBlock tex={String.raw`H^1(\xi_k) := \varprojlim_K H^1(X_{K, \bar{\BQ}}, \xi_k).`} />

This is a representation of $\GL_2(\BA_f) \times G\_\BQ$.

<Theorem type="theorem" label="Theorem 1 (Deligne–Carayol).">
...
<MathBlock tex={String.raw`\bigoplus_{\pi} \pi_f \otimes \varphi_\pi,`} />
...
<MathBlock tex={String.raw`\Sh(G, \mu)_K := G(\BQ) \backslash \big( \Hcal_\mu \times G(\BA_f) / K \big)`} />
...
<MathBlock tex={String.raw`H^i(\xi) := \varprojlim_K H^i\big( \Sh(G, \mu)_{K, \bar{\BQ}}, \bar{\BQ}_\ell \big),`} />

<MathBlock tex={String.raw`H^*(\xi) := \sum_i (-1)^i H^i(\xi).`} />

Here the sum defining $H^*(\xi)$ is taken in the Grothendieck group of $G(\BA_f) \times G_E$-representations.

<Theorem type="conjecture" label="Conjecture 2 (Kottwitz).">

In this Grothendieck group, we have (up to some twists)

<MathBlock tex={String.raw`H^*(\xi) = \sum_\pi a(\pi, \xi) \, \pi_f \otimes (R_\mu \circ \varphi_\pi).`} />
...
<MathBlock tex={String.raw`V_\ell := \varinjlim_{D} H^2(\Sht_{2, D}^{\mu} \times \bar{\eta}_2, \bar{\BQ}_\ell)`} />
